...
# ── Append this to the bottom of estyecomapp/models.py ────────────────────────

class BabySubCategory(models.Model):
    """
    Top-level navigation cards on the Baby page.
    e.g. Baby Clothes, Nursery Decor, Baby Toys …
    """
    name        = models.CharField(max_length=120)
    slug        = models.SlugField(unique=True, blank=True)
    description = models.TextField(blank=True)
    ...
    class Meta:
        ordering            = ["order", "name"]
        verbose_name        = "Baby Sub-Category"
        verbose_name_plural = "Baby Sub-Categories"
...
class BabyItem(models.Model):
    """
    A single baby product listed on the Baby page.
    Linked to one BabySubCategory.
    """
    sub_category    = models.ForeignKey(
        BabySubCategory,
        on_delete=models.CASCADE,
    ...
    discount_pct    = models.PositiveSmallIntegerField(
        default=0, help_text="Discount percentage e.g. 33"
    )
    ...
    has_free_delivery   = models.BooleanField(default=False)
    is_on_sale          = models.BooleanField(default=False)
    is_personalised     = models.BooleanField(default=False, help_text="Personalised/custom item")
    badge_label         = models.CharField(max_length=60, blank=True)
    low_stock_message   = models.CharField(
        max_length=80, blank=True,
        help_text="e.g. 'Only 2 left – order soon'"
    )
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            ...
            slug  = base
            n     = 1
            while BabyItem.objects.filter(slug=slug).exists():
                slug = f"{base}-{n}"
                n   += 1
            self.slug = slug
        if self.original_price and self.original_price > self.price_usd:
