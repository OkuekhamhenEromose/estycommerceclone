...
ALLOWED_HOSTS = config('ALLOWED_HOSTS', cast=Csv(), default='localhost,127.0.0.1,.railway.app')
...
# ========== REDIS CACHING ==========
if DEBUG:
    # Development: Use dummy cache that doesn't try to connect to Redis
    CACHES = {
        'default': {
    ...
    print("⚠️ Using dummy cache (no Redis)")
else:
...
# ========== DRF OPTIMIZATION ==========
if DEBUG:
    # Development: No throttling, no caching issues
    REST_FRAMEWORK = {
        ...
        'PAGE_SIZE': 20,
        # NO THROTTLING CLASSES IN DEVELOPMENT
        'DEFAULT_THROTTLE_CLASSES': [],
        'NUM_PROXIES': 1,
    }
else:
    # Production: With throttling
    REST_FRAMEWORK = {
        'DEFAULT_AUTHENTICATION_CLASSES': (
            'rest_framework_simplejwt.authentication.JWTAuthentication',
        ),
...
CSRF_TRUSTED_ORIGINS = config('CSRF_TRUSTED_ORIGINS', cast=Csv(), default='http://localhost:3000,https://*.railway.app')
